...
export function AppWidget({ id, state, metrics, goService, openAppWidget }) {
  ...
  return <><div className="widget-header"><h2><Icon size={16}/>{definition.name}</h2><span className="caption">NEXUS APPS</span></div><div className="native-widget-body" style={{'--app-color':definition.color}}><span className="native-widget-mark"><Icon size={29}/></span><div><h3>{id==='discord'?'Samen online':id==='whatsapp'?'Je gesprekken, dichtbij':'Jouw Spotify-bibliotheek'}</h3><p>{id==='spotify'&&metrics.spotify?.title?metrics.spotify.title: id==='discord'?'Servers, berichten en voice in je werkruimte.':id==='whatsapp'?'Chats en oproepen met je bestaande aanmelding.':'Playlists, podcasts en je opgeslagen muziek.'}</p></div><span className="native-widget-status"><i className={`status-dot ${installed?'':'neutral'}`}/>{installed?'Windows-app verbonden':'Windows-app vereist'}</span><div className="native-widget-actions"><button className="button primary" onClick={()=>openAppWidget(id)}><Monitor size={14}/>Open op dashboard</button><button className="button secondary" onClick={()=>goService(id)} aria-label={`${definition.name} volledig openen`}><ArrowUpRight size={15}/></button></div></div></>;
}
