...
try {
  ...
  await page.screenshot({ path: 'artifacts/1.1.0-library.png' });
  ...
  await page.screenshot({ path: 'artifacts/1.1.0-hardware.png' });
  ...
  await page.screenshot({ path: 'artifacts/1.1.0-radio.png' });
  await page.locator('.sidebar').getByRole('button', { name: 'Dashboard', exact: true }).click();
  assert.match(await page.locator('.radio-dock').textContent(), /LIVE RADIO/);
  await page.getByRole('button', { name: 'Radio pauzeren', exact: true }).click();
  ...
  await page.screenshot({ path: 'artifacts/1.1.0-rgb.png' });
  ...
  await page.screenshot({ path: 'artifacts/1.1.0-failure.png' }).catch(() => {}); throw error;
} finally {
